 ...
 *  - Checked here: the Baltimore passenger lists for 1874 (Bartholomäus, with
 *    his second wife Elisabetha and two children) and 1888 (Margaretha), the
 *    1900 census and Find a Grave.
 *  - The parents and the Bavarian details come from Find a Grave biographies
 *    and the "Frank and Shugars Families" member tree, which cites the
 *    Mistendorf parish registers and Baltimore church and civil records. They
 *    have not been checked against the registers, so they are UNPROVEN.
 */
'use strict';
...
const UNP = 'UNPROVEN: from the Find a Grave biographies and the "Frank and Shugars Families" member tree, which cites the Mistendorf parish registers; not yet checked against the registers.';

// ── Bartholomew Schriefer ───────────────────────────────────────────────────
edit('schriefer_bartholomew', [FAG_B, P1874, TREE], p => {
  ...
  note(p, 'Born 9 Sep 1839 at Zeegendorf, near Bamberg, Bavaria, and baptized "Joannes Bartholomaeus" in the parish of Mistendorf; died 8 Nov 1914 in Baltimore, aged 75, and buried at Most Holy Redeemer Cemetery with his third wife, Margaret (Find a Grave; the Frank and Shugars member tree).');
...
// ── Margaret Denzlein ───────────────────────────────────────────────────────
edit('denzlein_margaret', [FAG_M, P1888, TREE], p => {
  ...
  note(p, 'Born 19 May 1865 at Mistendorf, near Bamberg, Bavaria, and baptized there on 21 May; died 25 Oct 1924 at 917 Stirling Street, Baltimore, aged 59, and buried at Most Holy Redeemer Cemetery (Find a Grave; the Frank and Shugars member tree).');
...
person('schlasberger_joseph', 'Joseph Schlasberger', 'M', [FAG_B, TREE], p => {
  add(p.aliases, 'Joseph Schalsberger');
  add(p.locations, 'Zeegendorf, Bamberg, Bavaria');
  note(p, `${UNP} Named as the father of Bartholomew Schriefer (born 1839 at Zeegendorf); the son took his mother's surname, Schrüfer (Schriefer), which suggests his parents were not married when he was born. Spelled Schlasberger on Find a Grave and Schalsberger in the tree.`);
});
person('schruefer_margaretha', 'Margaretha Schrüfer', 'F', [FAG_B, TREE], p => {
  add(p.aliases, 'Margaretha Schriefer');
  add(p.locations, 'Zeegendorf, Bamberg, Bavaria');
  note(p, `${UNP} Named as the mother of Bartholomew Schriefer, born 9 Sep 1839 at Zeegendorf in the parish of Mistendorf.`);
});
child('schriefer_bartholomew', 'schlasberger_joseph', 'schruefer_margaretha');

// ── Margaret's parents ──────────────────────────────────────────────────────
person('denzlein_karl', 'Karl Denzlein', 'M', [FAG_M, TREE], p => {
  ...
  note(p, `${UNP} Born 1 Dec 1822 at house no. 5, Hochstall, and baptized the next day at the Church of the Assumption (Mariä Himmelfahrt), Mistendorf; a farmer at house no. 48, Mistendorf. Married Margaretha Popp there on 8 Feb 1847; their children included Margaretha (1849), Georg (1850–1851), Georg (1853), Margaretha (1860), Barbara (1862) and Margaret (1865). Alive when his wife died in 1877.`);
  note(p, 'UNPROVEN lead: the same tree names his parents as Johann Georg Denzlein (1796–1872) and Barbara Pfeufer (born 1794).');
});
person('popp_margaretha', 'Margaretha Popp (Denzlein)', 'F', [FAG_M, TREE], p => {
  ...
  note(p, `${UNP} Born 1821; married Karl Denzlein at Mistendorf on 8 Feb 1847; died there on 15 Oct 1877, when her daughter Margaret was 12.`);
});
