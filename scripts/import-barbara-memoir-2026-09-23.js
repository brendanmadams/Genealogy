#!/usr/bin/env node
/**
 * Apply the facts from "Barbara McKeldin Adams - A Memoir" approved by Brendan
 * Adams on 2026-09-23 (data/imports/barbara-memoir-2026-09-23.json): a gap check
 * that kept only facts the records lacked. Same rules as
 * import-bill-allen-2026-09-23.js; each fact cites its line in the memoir's
 * extracted text. Re-runnable.
 */
'use strict';
const fs = require('fs');
const path = require('path');
const { exists, load, save, note } = require('./lib/records');
const IMP = JSON.parse(fs.readFileSync(path.resolve(__dirname, '..', 'data', 'imports', 'barbara-memoir-2026-09-23.json'), 'utf8'));
...
const cite = f => ` [Barbara McKeldin Adams, A Memoir, text line ${f.lines.join(', ')}]`;
